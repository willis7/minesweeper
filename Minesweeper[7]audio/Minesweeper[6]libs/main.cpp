...
void drawScene()
{
	...
	glPushMatrix();	
		...
		}else
		{
			...
			SourcePos[3][0]=startX;							//sets the foostep sound position = player position
			SourcePos[3][1]=startY;
...
ALboolean loadALData()
{
	...
	alutLoadWAVFile("Data/footstep.wav", &format, &data, &size, &freq, &loop);
    alBufferData(Buffers[FOOTSTEP], format, data, size, freq);
    alutUnloadWAV(format, data, size, freq);
	...
	alSourcei (Sources[FOOTSTEP], AL_BUFFER,   Buffers[FOOTSTEP]  );
    alSourcef (Sources[FOOTSTEP], AL_PITCH,    1.0            );
    alSourcef (Sources[FOOTSTEP], AL_GAIN,     1.0            );
    alSourcefv(Sources[FOOTSTEP], AL_POSITION, SourcePos[FOOTSTEP]);
    alSourcefv(Sources[FOOTSTEP], AL_VELOCITY, SourceVel[FOOTSTEP]);
    alSourcei (Sources[FOOTSTEP], AL_LOOPING,  AL_FALSE       );
